...
$query_absen_hari_ini = mysqli_query($conn, "
    SELECT k.nama_lengkap, k.departemen, a.jam_masuk, a.jam_keluar 
    FROM tb_absensi a 
...
<head>
    ...
    <style>
        ...
        .btn-export { background-color: #198754; color: white; border-radius: 8px; font-weight: 600; padding: 12px 20px; text-decoration: none; font-size: 13.5px; border: none; transition: 0.2s;}
        .btn-export:hover { background-color: #157347; color: white;}
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="content-body">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div>
                    <h4 class="fw-bold mb-1 text-dark" style="font-size: 22px;">Laporan Kehadiran Harian</h4>
                    <p class="text-muted mb-0" style="font-size: 13px;">Pantau data absensi karyawan secara real-time dan unduh laporannya.</p>
                </div>
                <div class="d-flex gap-2">
                    <a href="export_excel.php" class="btn-export" style="background-color: #198754; color: white;"><i class="fas fa-file-excel me-2"></i> Excel</a>
                    <button onclick="cetakPDF()" class="btn-export" style="background-color: #dc3545; color: white; border: none;"><i class="fas fa-file-pdf me-2"></i> PDF</button>
                </div>
            </div>
            
            <div class="card-panel m-0 p-0 overflow-hidden" id="area-laporan">
                <div class="p-4 border-bottom" style="background-color: #fcfcfc;">
                    <h6 class="fw-bold mb-0" style="color: #004a8f;"><i class="fas fa-clipboard-list me-2"></i> Data Kehadiran: <?= date('d F Y'); ?></h6>
                </div>
                <div class="table-responsive">
                    <table class="table table-custom text-center mb-0">
                        <thead>
                            <tr><th class="text-start ps-4">Nama Lengkap</th><th>Departemen</th><th>Jam Masuk</th><th>Jam Keluar</th><th>Status</th></tr>
                        </thead>
                        <tbody>
                            <?php if(mysqli_num_rows($query_absen_hari_ini) > 0): ?>
                                <?php while($row = mysqli_fetch_assoc($query_absen_hari_ini)): ?>
                                    <?php 
                                        // LOGIKA HAKIM OTOMATIS (Batas 08:00)
                                    ...
                                    ?>
                                    <tr>
                                        <td class="text-start fw-bold text-dark ps-4"><?= $row['nama_lengkap']; ?></td>
                                        <td><?= $row['departemen']; ?></td>
                                        <td class="fw-bold"><?= date('H:i', strtotime($row['jam_masuk'])); ?></td>
                                        <td class="fw-bold"><?= $row['jam_keluar'] ? date('H:i', strtotime($row['jam_keluar'])) : 'Belum Pulang'; ?></td>
                                        <td><span class="badge <?= $status_warna; ?> bg-opacity-10 px-3 py-1 rounded-pill"><?= $status_teks; ?></span></td>
                                    </tr>
                                <?php endwhile; ?>
                            <?php else: ?>
                                <tr><td colspan="5" class="text-muted py-5">Belum ada karyawan yang melakukan absensi masuk hari ini.</td></tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                </div>
    ...
    <script>
        function cetakPDF() {
            // Mengambil elemen tabel yang tadi kita beri ID
            var elemenTabel = document.getElementById('area-laporan');
            
            // Pengaturan hasil PDF
            var opsi = {
                margin:       0.5, // Margin kertas
                filename:     'Laporan_Kehadiran_FoodSync.pdf', // Nama file saat terdownload
                image:        { type: 'jpeg', quality: 0.98 },
                html2canvas:  { scale: 2 }, // Agar gambar/teks PDF tidak pecah (HD)
                jsPDF:        { unit: 'in', format: 'A4', orientation: 'landscape' } // Kertas A4 menyamping
            };

            // Proses mengubah HTML jadi PDF lalu otomatis mendownloadnya
            html2pdf().set(opsi).from(elemenTabel).save();
        }
    </script>
</body>
</html>