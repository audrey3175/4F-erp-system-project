...
if(isset($_POST['simpan_karyawan'])) {
    ...
    if(mysqli_num_rows($cek_email) > 0) {
        $notif_error = 'Gagal! Email tersebut sudah terdaftar di sistem.';
    } else {
        $id_baru = 'KRY-' . rand(1000, 9999);
        $nik_baru = rand(10000000, 99999999);

        $query_insert = "INSERT INTO karyawan (id_karyawan, nik, nama_lengkap, email, password, departemen, jabatan, role_akun) 
                         VALUES ('$id_baru', '$nik_baru', '$nama_lengkap', '$email', '$password_hash', '$departemen', '$jabatan', '$role_akun')";
        if(mysqli_query($conn, $query_insert)) {
            $notif_sukses = 'Berhasil! Karyawan baru telah disimpan ke database.';
        } else { $notif_error = 'Sistem Error: ' . mysqli_error($conn); }
    }
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Tambah Karyawan - FoodSync</title>
    ...
    <style>
        ...
        .form-label { font-size: 12px; font-weight: 700; color: #888; text-transform: uppercase; margin-bottom: 8px;}
        .form-control, .form-select { border-radius: 8px; border: 1px solid #e0e0e0; padding: 12px 15px; font-size: 13.5px; background-color: #fcfcfc;}
        .form-control:focus, .form-select:focus { border-color: #004a8f; box-shadow: none; background-color: #fff;}
        .section-title { font-size: 15px; font-weight: 800; color: #333; margin-bottom: 20px; padding-bottom: 10px; border-bottom: 1px solid #f0f0f0;}
        .btn-submit { background-color: #004a8f; color: white; border: none; padding: 12px 30px; border-radius: 8px; font-weight: 700; font-size: 14px; transition: 0.2s;}
    ...
    <div class="main-content">
        ...
        <div class="content-body">
            <div class="d-flex align-items-center gap-3 mb-4">
                ...
                <div>
                    <h4 class="fw-bold mb-1 text-dark" style="font-size: 22px;">Tambah Karyawan Baru</h4>
                    <p class="text-muted mb-0" style="font-size: 13px;">Lengkapi formulir di bawah ini untuk mendaftarkan akun baru.</p>
                </div>
            </div>
            ...
            <div class="row g-4">
                <div class="col-lg-8">
                    <div class="card-panel m-0">
                        <form method="POST">
                            <div class="section-title"><i class="far fa-address-card text-primary me-2"></i> Informasi Personal & Akun</div>
                            <div class="row g-3 mb-4">
                                <div class="col-md-6"><label class="form-label">Nama Lengkap</label><input type="text" name="nama_lengkap" class="form-control" placeholder="Cth: Budi Santoso" required></div>
                                <div class="col-md-6"><label class="form-label">Email Perusahaan</label><input type="email" name="email" class="form-control" placeholder="Cth: budi@foodsync.com" required></div>
                                <div class="col-md-12"><label class="form-label">Password Sementara</label><input type="password" name="password" class="form-control" placeholder="Buat password login awal" required></div>
                            </div>
                            
                            <div class="section-title mt-2"><i class="fas fa-sitemap text-primary me-2"></i> Posisi & Hak Akses</div>
                            <div class="row g-3 mb-4">
                                <div class="col-md-4">
                                ...
                                <div class="col-md-4"><label class="form-label">Jabatan Spesifik</label><input type="text" name="jabatan" class="form-control" placeholder="Cth: Staff Produksi" required></div>
                                <div class="col-md-4">
                                    <label class="form-label">Hak Akses Sistem</label>
                                    <select name="role_akun" class="form-select" required>
                                        <option value="Karyawan">Karyawan (Standar)</option>
                            ...
                            <button type="submit" name="simpan_karyawan" class="btn-submit w-100"><i class="fas fa-save me-2"></i> Simpan Data Karyawan</button>
                        </form>
                    </div>
                </div>

                <div class="col-lg-4">
                    <div class="card-panel m-0 mb-4" style="background-color: #f8f9fa;">
                        <h6 class="fw-bold mb-3 text-dark" style="font-size: 14px;"><i class="fas fa-info-circle text-primary me-2"></i> Panduan Input</h6>
                        <ul class="text-muted mb-0 ps-3" style="font-size: 12px; line-height: 1.6;">
                            <li class="mb-2"><strong>ID Karyawan & NIK</strong> akan digenerate otomatis oleh sistem FoodSync ERP.</li>
                            <li class="mb-2">Pilih <strong>Hak Akses Supervisor</strong> jika akun ini bertugas untuk melakukan ACC/Tolak pengajuan cuti dan reimburse.</li>
                            <li>Pastikan Anda memberikan <strong>Password Sementara</strong> kepada karyawan bersangkutan agar mereka bisa Login.</li>
                        </ul>
                    </div>
                </div>
            </div>
