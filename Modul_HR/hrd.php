...
$query_karyawan = mysqli_query($conn, "SELECT * FROM karyawan WHERE nama_lengkap IS NOT NULL AND nama_lengkap != '' ORDER BY id_karyawan DESC");
?>
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <title>Panel HRD - FoodSync</title>
    ...
    <style>
        ...
        .btn-add { background-color: #004a8f; color: white; border-radius: 8px; font-weight: 600; padding: 10px 20px; text-decoration: none; font-size: 13.5px; transition: 0.2s; border: none;}
        .btn-add:hover { background-color: #003366; color: white;}
        ...
        .table-custom td { padding: 15px; font-size: 13px; vertical-align: middle; cursor: pointer; color: #444; border-bottom: 1px solid #f8f9fa;}
        .table-custom tr { transition: 0.2s; }
    ...
    <div class="main-content">
        ...
        <div class="content-body">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div>
                    <h4 class="fw-bold mb-1 text-dark" style="font-size: 22px;">Data Karyawan</h4>
                    <p class="text-muted mb-0" style="font-size: 13px;">Kelola direktori database dan status seluruh karyawan.</p>
                </div>
                <a href="hrd_tambah_karyawan.php" class="btn-add"><i class="fas fa-user-plus me-2"></i> Tambah Karyawan</a>
            </div>
            ...
            <div class="card-panel m-0 p-0 overflow-hidden">
                <div class="table-responsive">
                    <table class="table table-custom text-center mb-0">
                        <thead>
                            <tr>
                                <th class="text-start ps-4">Nama Pegawai</th>
                                <th>ID Akun</th>
                                <th>Departemen</th>
                                <th>Jabatan</th>
                                <th>Role Akses</th>
                        ...
                        <tbody>
                            <?php while($row = mysqli_fetch_assoc($query_karyawan)): ?>
                                <!-- Baris ini bisa diklik untuk edit/hapus (opsional) -->
                                <tr onclick="window.location.href='hrd_detail_karyawan.php?id=<?= $row['id_karyawan']; ?>'">
                                    <td class="text-start fw-bold text-dark ps-4 d-flex align-items-center">
                                        <div style="width: 35px; height: 35px; background: #e8f0fe; color: #004a8f; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: bold; margin-right: 15px; font-size: 14px;">
                                            <?= substr($row['nama_lengkap'], 0, 1); ?>
                                        </div>
                                        <div>
                                            <div class="mb-0"><?= $row['nama_lengkap']; ?></div>
                                            <div class="text-muted fw-normal" style="font-size: 11px;"><?= $row['email']; ?></div>
                                        </div>
                                    </td>
                                    <td class="fw-bold" style="color: #666;"><?= $row['id_karyawan']; ?></td>
                                    <td><?= $row['departemen']; ?></td>
                                    <td><?= $row['jabatan']; ?></td>
                                    <td>
                                        <?php if($row['role_akun'] == 'HRD' || $row['role_akun'] == 'Admin'): ?>
                                            <span class="badge bg-danger bg-opacity-10 text-danger rounded-pill px-3 py-1"><?= $row['role_akun']; ?></span>
                                        <?php elseif($row['role_akun'] == 'Supervisor'): ?>
                                            <span class="badge bg-warning bg-opacity-10 text-dark rounded-pill px-3 py-1"><?= $row['role_akun']; ?></span>
                                        <?php else: ?>
                                            <span class="badge bg-primary bg-opacity-10 text-primary rounded-pill px-3 py-1"><?= $row['role_akun']; ?></span>
                                        <?php endif; ?>
                                    </td>
                                </tr>
                            <?php endwhile; ?>
