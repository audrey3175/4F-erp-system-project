...
if (isset($_POST['ajukan_reimburse'])) {
    // Hilangkan titik dari format input (contoh: 1.500.000 menjadi 1500000)
    $nominal_mentah = str_replace('.', '', $_POST['nominal']);
    $nominal = mysqli_real_escape_string($conn, $nominal_mentah);
    $keterangan = mysqli_real_escape_string($conn, $_POST['keterangan']);
    
    $nama_file = $_FILES['bukti_foto']['name'];
    $ukuran_file = $_FILES['bukti_foto']['size'];
    $tmp_name = $_FILES['bukti_foto']['tmp_name'];
    $error_file = $_FILES['bukti_foto']['error'];
    
    $nama_file_baru = ''; // Default kosong jika tidak upload
    $upload_ok = true;

    // Jika user mengunggah file (error 0)
    if ($error_file === 0) {
        $ekstensi_valid = ['jpg', 'jpeg', 'png', 'pdf'];
        $ekstensi_file = strtolower(pathinfo($nama_file, PATHINFO_EXTENSION));
        
        if (in_array($ekstensi_file, $ekstensi_valid)) {
            if ($ukuran_file < 10000000) { // Limit dinaikkan jadi 10MB
                $nama_file_baru = uniqid() . '.' . $ekstensi_file;
                $tujuan_upload = 'uploads/' . $nama_file_baru;
                
                if (!move_uploaded_file($tmp_name, $tujuan_upload)) {
                    $upload_ok = false;
                    $notif_pesan = "Gagal memindahkan file foto ke folder server."; $notif_tipe = "danger";
                }
            } else { 
                $upload_ok = false;
                $notif_pesan = "File terlalu besar! Maksimal 10MB."; $notif_tipe = "warning"; 
            }
        } else { 
            $upload_ok = false;
            $notif_pesan = "Format file tidak didukung! (Hanya JPG, PNG, PDF)."; $notif_tipe = "warning"; 
        }
    } elseif ($error_file !== 4) {
        // Error 4 artinya tidak ada file yang diupload (OPSIONAL), itu boleh. Selain 4, berarti ada error sistem.
        $upload_ok = false;
        $notif_pesan = "Terjadi kesalahan sistem saat membaca file."; $notif_tipe = "danger";
    }

    // Jika proses validasi lolos (baik upload file maupun tidak upload file)
    if ($upload_ok) {
        $query_reimburse = "INSERT INTO tb_reimbursement (id_karyawan, nominal, keterangan, bukti_foto, status) VALUES ('$id_karyawan', '$nominal', '$keterangan', '$nama_file_baru', 'Pending')";
        if (mysqli_query($conn, $query_reimburse)) {
            $notif_pesan = "Klaim reimbursement berhasil dikirim."; $notif_tipe = "success";
        } else { 
            $notif_pesan = "Gagal menyimpan ke database."; $notif_tipe = "danger"; 
        }
    }
}
$query_riwayat = mysqli_query($conn, "SELECT * FROM tb_reimbursement WHERE id_karyawan = '$id_karyawan' ORDER BY id_reimburse DESC");
?>
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <title>Reimbursement - FoodSync</title>
    ...
    <style>
        ...
        .form-control { border-radius: 8px; border: 1px solid #e0e0e0; padding: 12px; font-size: 13.5px; background-color: #fcfcfc;}
        .form-control:focus { border-color: #004a8f; box-shadow: none; background-color: #fff;}
        .btn-submit { background-color: #004a8f; color: white; border: none; padding: 12px 25px; border-radius: 8px; font-weight: 600; transition: 0.2s;}
        .btn-submit:hover { background-color: #003366; }
        
        .table-custom th { font-size: 12px; color: #888; text-transform: uppercase; font-weight: 600; border-bottom: 2px solid #f0f0f0; padding-bottom: 15px;}
        .table-custom td { padding: 15px 0; font-size: 13.5px; color: #444; border-bottom: 1px solid #f8f9fa; vertical-align: middle;}
       ...
       <div class="top-bar">
        ...
        <div class="content-body">
            ...
            <div class="d-flex align-items-center gap-3 mb-4">
                <a href="karyawan.php" class="btn-back-icon" title="Kembali ke Portal Karyawan"><i class="fas fa-arrow-left"></i></a>
                <div>
                    <h4 class="fw-bold mb-1 text-dark" style="font-size: 22px;">Klaim Reimbursement</h4>
                    <p class="text-muted mb-0" style="font-size: 13px;">Ajukan penggantian dana operasional. Upload bukti struk jika ada.</p>
                </div>
            </div>
            ...
            <div class="row g-4">
                <div class="col-lg-5">
                    <div class="card-panel m-0">
                        <form method="POST" enctype="multipart/form-data">
                            <div class="mb-3">
                                <label class="form-label fw-bold text-muted" style="font-size: 12px; text-transform:uppercase;">Total Nominal (Rp)</label>
                                <input type="text" name="nominal" id="inputNominal" class="form-control" placeholder="Cth: 150.000" required>
                            </div>
                            <div class="mb-3">
                                <label class="form-label fw-bold text-muted" style="font-size: 12px; text-transform:uppercase;">Upload Bukti Struk <span class="text-primary text-lowercase fw-normal">(Opsional)</span></label>
                                <input type="file" name="bukti_foto" class="form-control">
                                <small class="text-muted mt-1 d-block" style="font-size: 11px;">Maksimal 10MB (JPG, PNG, PDF).</small>
                            </div>
                            <div class="mb-4">
                                <label class="form-label fw-bold text-muted" style="font-size: 12px; text-transform:uppercase;">Rincian Pengeluaran</label>
                                <textarea name="keterangan" class="form-control" rows="3" placeholder="Sebutkan rinciannya dengan jelas..." required></textarea>
                            </div>
                            <button type="submit" name="ajukan_reimburse" class="btn-submit w-100"><i class="fas fa-paper-plane me-2"></i> Kirim Klaim</button>
                        </form>
                    </div>
                </div>
                ...
                <div class="col-lg-7">
                    <div class="card-panel m-0">
                        <h6 class="fw-bold mb-4 text-dark" style="font-size: 15px;">Riwayat Reimbursement</h6>
                        <div class="table-responsive">
                            <table class="table table-custom text-center mb-0">
                                <thead><tr><th>Nominal</th><th>Keterangan</th><th>Bukti</th><th>Status</th></tr></thead>
                                <tbody>
                                    <?php if(mysqli_num_rows($query_riwayat) > 0): ?>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <?php if($row['status'] == 'Pending'): ?>
                                                        <span class="badge bg-warning text-dark px-3 py-1 rounded-pill">Menunggu</span>
                                        ...
                                        <?php endwhile; ?>
                                    <?php else: ?>
                                        <tr><td colspan="4" class="text-muted py-4">Belum ada riwayat klaim.</td></tr>
                                    <?php endif; ?>
                                </tbody>
                            </table>
                        </div>
