...
// Mengambil Statistik
$query_total = mysqli_query($conn, "SELECT COUNT(*) as total FROM karyawan WHERE nama_lengkap IS NOT NULL AND nama_lengkap != ''");
$data_total = mysqli_fetch_assoc($query_total);
$total_karyawan = $data_total ? $data_total['total'] : 0;

$query_dept = mysqli_query($conn, "SELECT COUNT(DISTINCT departemen) as total_dept FROM karyawan WHERE departemen IS NOT NULL AND departemen != ''");
$data_dept = mysqli_fetch_assoc($query_dept);
$total_dept = $data_dept ? $data_dept['total_dept'] : 0;

// Logika Hero Banner menyesuaikan Role (Lebih Adil dan General)
$link_utama = 'karyawan.php';
$teks_utama = 'Buka Portal Karyawan';
$icon_utama = 'fa-user-check';

if ($role == 'HRD' || $role == 'Admin') { $link_utama = 'hrd.php'; $teks_utama = 'Buka Panel HRD'; $icon_utama = 'fa-id-badge'; }
else if ($role == 'Supervisor') { $link_utama = 'supervisor.php'; $teks_utama = 'Kelola Tim Departemen'; $icon_utama = 'fa-users'; }
else if ($role == 'Finance') { $link_utama = 'payroll.php'; $teks_utama = 'Proses Payroll Karyawan'; $icon_utama = 'fa-money-check-alt'; }
?>
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <title>Dashboard - FoodSync ERP</title>
    ...
    <style>
        ...
        /* KOMPONEN DASHBOARD */
        .hero-card { background: linear-gradient(135deg, #004a8f, #002d5c); border-radius: 16px; padding: 40px; color: white; margin-bottom: 25px; position: relative; overflow: hidden;}
        .hero-card::after { content: ''; position: absolute; right: -50px; top: -50px; width: 250px; height: 250px; background: rgba(255,255,255,0.05); border-radius: 50%; }
        .hero-title { font-size: 28px; font-weight: 800; margin-bottom: 10px; letter-spacing: -0.5px;}
        .hero-subtitle { font-size: 14px; opacity: 0.9; margin-bottom: 30px; line-height: 1.6; max-width: 600px;}
        .btn-hero { background-color: white; color: #004a8f; border: none; padding: 12px 25px; border-radius: 8px; font-weight: 700; font-size: 14px; text-decoration: none; display: inline-block; transition: 0.3s; box-shadow: 0 4px 10px rgba(0,0,0,0.1);}
        .btn-hero:hover { background-color: #f8f9fa; transform: translateY(-2px); color: #004a8f;}
        
        .stat-icon-wrap { width: 50px; height: 50px; border-radius: 12px; display: flex; align-items: center; justify-content: center; font-size: 20px; margin-right: 15px;}
        .stat-val { font-size: 26px; font-weight: 800; color: #222; line-height: 1.2;}
        ...
        .timeline-item { position: relative; padding-left: 20px; margin-bottom: 15px; border-left: 2px solid #e8f0fe;}
        .timeline-item::before { content: ''; position: absolute; left: -6px; top: 0; width: 10px; height: 10px; border-radius: 50%; background-color: #004a8f;}
        .tl-date { font-size: 11px; color: #888; font-weight: 600; margin-bottom: 3px;}
        .tl-text { font-size: 13px; color: #444; font-weight: 500;}
    </style>
</head>
<body>
    ...
    <div class="main-content">
        ...
        <div class="content-body">
            <div class="mb-4">
                <h4 class="fw-bold mb-1 text-dark" style="font-size: 22px;">Portal Dashboard</h4>
                <p class="text-muted" style="font-size: 13px;">Ringkasan informasi dan jalan pintas sistem perusahaan Anda.</p>
            </div>

            <div class="hero-card">
                <div class="hero-title" style="position: relative; z-index: 2;">Selamat Datang di FoodSync ERP</div>
                <div class="hero-subtitle" style="position: relative; z-index: 2;">Sistem Terintegrasi Perencanaan Sumber Daya Perusahaan. Anda masuk menggunakan hak akses <strong><?= $role; ?></strong>.</div>
                
                <a href="<?= $link_utama; ?>" class="btn-hero" style="position: relative; z-index: 2;"><i class="fas <?= $icon_utama; ?> me-2"></i> <?= $teks_utama; ?></a>
            </div>

            <div class="row g-4 mb-4">
                <div class="col-md-4">
                    <div class="card-panel h-100 d-flex align-items-center m-0">
                        <div class="stat-icon-wrap" style="background-color: #e8f0fe; color: #004a8f;"><i class="fas fa-building"></i></div>
                        <div><div class="stat-val"><?= $total_dept; ?></div><div class="stat-label">Total Departemen</div></div>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="card-panel h-100 d-flex align-items-center m-0">
                        <div class="stat-icon-wrap" style="background-color: #e8f5e9; color: #2e7d32;"><i class="fas fa-users"></i></div>
                        <div><div class="stat-val"><?= $total_karyawan; ?></div><div class="stat-label">Karyawan Aktif</div></div>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="card-panel h-100 d-flex align-items-center m-0">
                        <div class="stat-icon-wrap" style="background-color: #fff8e1; color: #f57f17;"><i class="fas fa-server"></i></div>
                        <div><div class="stat-val text-success" style="font-size: 20px;">Online</div><div class="stat-label">Status Database</div></div>
                    </div>
                </div>
            </div>
            ...
            <div class="row g-4">
                <div class="col-lg-6">
                    <div class="card-panel h-100 m-0">
                        <div class="table-title"><i class="fas fa-bullhorn text-primary me-2 opacity-75"></i> Pengumuman Perusahaan</div>
                        <div class="timeline-item">
                            <div class="tl-date">16 Juni 2026</div>
                            <div class="tl-text">Implementasi Sistem ERP FoodSync fase pertama berjalan sukses untuk seluruh departemen.</div>
                        </div>
                        <div class="timeline-item">
                            <div class="tl-date">10 Juni 2026</div>
                            <div class="tl-text">Pembaruan sistem database dan penyesuaian UI Dashboard Karyawan terbaru.</div>
                        </div>
                    </div>
                </div>
                <div class="col-lg-6">
                    <div class="card-panel h-100 m-0">
                        <div class="table-title"><i class="fas fa-shield-alt text-primary me-2 opacity-75"></i> Informasi Akun Anda</div>
                        <table class="table table-borderless table-sm mb-0" style="font-size: 13px;">
                            <tr><td class="text-muted fw-bold" width="130">Nama Lengkap</td><td>: <span class="fw-bold text-dark"><?= $user_name; ?></span></td></tr>
                            <tr><td class="text-muted fw-bold">Hak Akses / Role</td><td>: <span class="badge bg-primary bg-opacity-10 text-primary px-2 py-1"><?= $role; ?></span></td></tr>
                            <tr><td class="text-muted fw-bold">Departemen</td><td>: <?= $departemen; ?></td></tr>
                            <tr><td class="text-muted fw-bold">Zona Waktu</td><td>: WIB (Asia/Jakarta)</td></tr>
                        </table>
                    </div>
                </div>
            </div>
