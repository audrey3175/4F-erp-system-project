...
if (isset($_POST['ajukan_cuti'])) {
    ...
    if ($tgl_mulai > $tgl_selesai) {
        $notif_pesan = "Tanggal selesai tidak boleh lebih awal dari tanggal mulai!"; $notif_tipe = "danger";
    } else {
        $query_cuti = "INSERT INTO tb_cuti (id_karyawan, tgl_mulai, tgl_selesai, alasan, status) VALUES ('$id_karyawan', '$tgl_mulai', '$tgl_selesai', '$alasan', 'Pending')";
        if (mysqli_query($conn, $query_cuti)) {
            $notif_pesan = "Pengajuan cuti berhasil dikirim ke Supervisor."; $notif_tipe = "success";
        } else {
            $notif_pesan = "Gagal mengirim pengajuan: " . mysqli_error($conn); $notif_tipe = "danger";
        }
    }
}
$query_riwayat = mysqli_query($conn, "SELECT * FROM tb_cuti WHERE id_karyawan = '$id_karyawan' ORDER BY id_cuti DESC");
?>
<!DOCTYPE html>
<html lang="id">
<head>
    ...
    <title>Pengajuan Cuti - FoodSync</title>
    ...
    <div class="main-content">
        ...
        <div class="content-body">
            ...
            <div class="d-flex align-items-center gap-3 mb-4">
                ...
                <div>
                    <h4 class="fw-bold mb-1 text-dark" style="font-size: 22px;">Pengajuan Cuti</h4>
                    <p class="text-muted mb-0" style="font-size: 13px;">Isi formulir di bawah ini untuk memproses permohonan Anda.</p>
                </div>
            </div>
            ...
            <div class="row g-4">
                <div class="col-lg-5">
                    <div class="card-panel m-0">
                        <form method="POST">
                            <div class="mb-3">
                                <label class="form-label fw-bold text-muted" style="font-size: 12px; text-transform:uppercase;">Tanggal Mulai Cuti</label>
                                <input type="date" name="tgl_mulai" class="form-control" required>
                            </div>
                            <div class="mb-3">
                                <label class="form-label fw-bold text-muted" style="font-size: 12px; text-transform:uppercase;">Tanggal Selesai Cuti</label>
                                <input type="date" name="tgl_selesai" class="form-control" required>
                            </div>
                            <div class="mb-4">
                                <label class="form-label fw-bold text-muted" style="font-size: 12px; text-transform:uppercase;">Alasan Cuti / Keterangan</label>
                                <textarea name="alasan" class="form-control" rows="3" placeholder="Sebutkan alasan yang jelas..." required></textarea>
                            </div>
                            <button type="submit" name="ajukan_cuti" class="btn-submit w-100"><i class="fas fa-paper-plane me-2"></i> Kirim Pengajuan</button>
                        </form>
                    </div>
                </div>
                ...
                <div class="col-lg-7">
                    <div class="card-panel m-0">
                        <h6 class="fw-bold mb-4 text-dark" style="font-size: 15px;">Riwayat Pengajuan Cuti</h6>
                        <div class="table-responsive">
                            <table class="table table-custom text-center mb-0">
                                <thead><tr><th>Tgl Mulai</th><th>Tgl Selesai</th><th>Alasan</th><th>Status</th></tr></thead>
                                <tbody>
                                    <?php if(mysqli_num_rows($query_riwayat) > 0): ?>
                                        <?php while($row = mysqli_fetch_assoc($query_riwayat)): ?>
                                            <tr>
                                                <td class="fw-bold text-dark"><?= date('d/m/Y', strtotime($row['tgl_mulai'])); ?></td>
                                                <td class="fw-bold text-dark"><?= date('d/m/Y', strtotime($row['tgl_selesai'])); ?></td>
                                        ...
                                        <?php endwhile; ?>
                                    <?php else: ?>
                                        <tr><td colspan="4" class="text-muted py-4">Belum ada riwayat pengajuan cuti.</td></tr>
                                    <?php endif; ?>
                                </tbody>
                            </table>
                        </div>
